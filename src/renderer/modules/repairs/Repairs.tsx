...
export function Repairs(): JSX.Element {
  ...
  function openDetails(ticket: RepairTicket): void {
    ...
    setLaborDraft(String(ticket.laborFee));
  }

  function handleSavePartsAndLabor(): void {
    if (!selectedTicket) return;
    updatePartsAndLabor(selectedTicket.id, partsDraft.trim(), Number(laborDraft) || 0);
  }
      ...
      {selectedTicket ? (
        <div className="card" style={{ marginTop: 24 }}>
          ...
          <div className="form-row">
            ...
            <div>
              <label htmlFor="rep-labor">اجرت (تومان)</label>
              <input id="rep-labor" type="number" min={0} value={laborDraft} onChange={(e) => setLaborDraft(e.target.value)} />
            </div>
          </div>
